...
public final class TokenHandler {
	...
	public RequestUser parseUserFromToken(String token) {
		...
		if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
			try {
				...
				if (validHash) {
					final RequestUser user = fromJSON(userBytes);
	...
	public String createTokenForUser(RequestUser user) {
		user.setSenha("");
	...
	private RequestUser fromJSON(final byte[] userBytes) {
		try {
			return new ObjectMapper().readValue(new ByteArrayInputStream(userBytes), RequestUser.class);
		} catch (IOException e) {
			throw new IllegalStateException(e);
		}
	}

	private byte[] toJSON(RequestUser user) {
		try {
